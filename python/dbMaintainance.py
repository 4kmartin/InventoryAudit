#! /bin/python3
"""
    This module contains all the functions needed to maintain the Database
"""
...
def _run_select_statement(connection: Connection, statement: str) -> list[Any]:
    try:
...
def insert_select(
        ...
        table: str, select_statement: str,
...
def create_asset_table(connection: Connection, table_name: str = "discovered_assets",
                       table_header: tuple[str, ...] = ("source", "date_found", "hostname", "fqdn", "ip", "mac")
                       ) -> None:
...
def migrate_to_historical_data(connection: Connection) -> None:
    ...
    insert_select(connection, "historical_data", statement, None)
...
def update_report_item_single_item(connection: Connection, field: str, value: Any, rowid: int,
                                   table_name: str = "reportable_data") -> None:
    statement = "\n".join(
        (
            f"UPDATE {table_name}",
            f"SET {field} = ?",
...
def iterate_list_of_assets_with_null_field(connection: Connection,
                                           function_to_call: Callable[[Connection, tuple[Any, ...], str], None],
                                           asset_list: list[tuple[Any]], null_field: str) -> None:
    for asset in asset_list:
        function_to_call(connection, asset, null_field)


def lookup_matching_values_then_update_null_fields(connection: Connection, asset: tuple[Any, ...],
                                                   null_field: str) -> None:
...
def append_data_to_field(connection: Connection, asset: tuple[Any, ...], field: str,
                         table_name: str = "reportable_data") -> None:
    (rowid, lookup) = map_lookup_to_values(asset)
...
def map_lookup_to_values(asset: tuple[Any, ...]) -> tuple[int, dict[str, Any]]:
    return (asset[0],
            {
                "hostname": asset[1],
                "fqdn": asset[2],
...
def fill_nulls(connection: Connection) -> None:
    for field in ["hostname", "fqdn", "ip", "mac"]:
        ...
        iterate_list_of_assets_with_null_field(connection, lookup_matching_values_then_update_null_fields, assets,
                                               field)
...
def fill_in_inventory(connection: Connection, primary_inventory: str) -> None:
    collected_sources = _run_select_statement(connection, "SELECT rowid, source FROM reportable_data")
    for (rowid, sources) in collected_sources:
