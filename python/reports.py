...
def report_new_assets (connection:Connection) -> list[tuple]:
    ...
    statement = f"SELECT hostname,fqdn,ip,mac FROM discovered_assets WHERE date = {today} EXCEPT SELECT hostname,fqdn,ip,mac FROM discovered_assets WHERE date < {today}"
